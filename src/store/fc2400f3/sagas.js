...
function* loadRecordsMetaDataSaga() {
    while (true) { // eslint-disable-line no-constant-condition
        ...
        try {
            const company = yield call(loadRecordsMetaData, 'company')
            let recordsMetaData = { companyOptions: company.data && company.data.length > 0 && company.data.map(c => Object.assign({}, { label: c.name, value: c._id })) || [] }
