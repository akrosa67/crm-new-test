...
export function deleteRecord(id) {
    return api.put(`/delete_${atob(constantName)}_by_id`,{"processStatusId":[id],"status":3}).catch((error) => Promise.reject(error))
}
 ...
 */
 export function updateStatus(record) {
    return api.put(`/delete_${atob(constantName)}_by_id`, {"processStatusId":record.sub_sourceId,"status":record.status}).catch((error) => Promise.reject(error))
}
