...
function* loadRecordsMetaDataSaga() {
    while (true) { // eslint-disable-line no-constant-condition
        ...
        try {
            const department = yield call(loadRecordsMetaData, 'department')
            let recordsMetaData = { departmentOptions: department.data && department.data.length > 0 && department.data.map(c => Object.assign({}, { label: c.name, value: c._id })) || [] }
