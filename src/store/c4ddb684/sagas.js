...
function* loadRecordsMetaDataSaga() {
    while (true) { // eslint-disable-line no-constant-condition
        ...
        try {
            ...
            const menuHeaderOptions = (menuHeaderFilter || []).map(opt => Object.assign({}, opt, { label: opt.menuName, value: opt._id}))
            const menuSubHeaderOptions = (menuSubHeaderFilter || []).map(opt =>  Object.assign({}, opt, { label: opt.menuName, value: opt._id}))
           
            let recordsMetaData = { accessOptions: access.data && access.data.length > 0 && access.data.map(c => Object.assign({}, { label: c.accessName, value: c._id })) || [], accessList:  (access.data && access.data.length > 0 && access.data) || [], menuHeaderOptions, menuSubHeaderOptions }
