...
export function loadRecord(id) {
    return api.get(`/get_${atob(constantName)}_by_id?priorityId=${id}`).then((response) => response.data).catch((error) => Promise.reject(error))
}
...
export function deleteRecord(id) {
    const record ={"priorityId":[id],"status":3}
