...
function* createRecordSaga() {
    while (true) { // eslint-disable-line no-constant-condition
        ...
        if (create) {
            ...
            try {
                yield call(createRecord, Object.assign({}, record, { accMain: true, isNew: record.isNew || false }))
...
function* deleteRecordSaga() {
    while (true) { // eslint-disable-line no-constant-condition
        ...
        if (del) {
            ...
            try {
                yield call(deleteRecord, {"menuId":[id],"status":3})
                yield delay(1000)
                yield put(loadRecordsAction(true))
...
function* loadRecordsMetaDataSaga() {
    while (true) { // eslint-disable-line no-constant-condition
        ...
        try {
            const access = yield call(loadRecordsMetaData, 'access')
            const menu = yield call(loadRecordsMetaData, 'menu', { status: 'active' })
            const menuOptions = menu && menu.data && menu.data.length > 0 ? menu.data : []
            const menuHeaderFilter = ((menuOptions || []).find(opt => opt._id === "menuHeader") && (menuOptions || []).find(opt => opt._id === "menuHeader").menu) || []
            const menuSubHeaderFilter = ((menuOptions || []).find(opt => opt._id === "menuSubHeader") && (menuOptions || []).find(opt => opt._id === "menuSubHeader").menu) || []
            const menuHeaderOptions = (menuHeaderFilter || []).map(opt => Object.assign({}, { label: opt.menuName, value: opt._id}))
            const menuSubHeaderOptions = (menuHeaderFilter || []).reduce((a, el) => {
                const menuSubHeaderOpt = (menuSubHeaderFilter || []).filter(s => s.menuReferenceId.toString() === el._id.toString()) || []
                if(menuSubHeaderOpt && menuSubHeaderOpt.length > 0){
                    a.push(Object.assign({}, { label: el.menuName, options: (menuSubHeaderOpt || []).map(opt => Object.assign({}, { label: capitalizeFirstLetter(opt.menuName), value: opt._id})) }))
                }
                return a
            }, [])
            
            let recordsMetaData = { accessOptions: access.data && access.data.length > 0 && access.data.map(c => Object.assign({}, { label: c.accessName, value: c._id })) || [], menuHeaderOptions, menuSubHeaderOptions  }
