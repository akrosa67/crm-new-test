...
export function loadRecords() {
    return api.post(`/get_all_${atob(constantName)}`, {status: 'active'}).then((response) => response.data).catch((error) => Promise.reject(error))
}
 ...
 * @returns 
 */
export function loadRecordsMetaData(metaData, data = {}) {
    return api.post(`/get_all_${metaData}`, data).then((response) => response.data).catch((error) => Promise.reject(error))
}
